...
@section('content')
    ...
    <div class="p-4 m-4 sm:p-8 bg-white shadow sm:rounded-lg ">
        ...
        <div class="max-w-xl">
            ...
            <form action="{{ route('student.update', $student->id) }}" method="post" enctype="multipart/form-data" onsubmit="formatBeforeSubmit()">
                @csrf
                @method('PUT')
                ...
                <input id="passport" name="passport" value="{{ old('passport', $student->passport) }}" type="text"
                       class="form-control">
                ...
                <input id="parents_name" name="parents_name" value="{{ old('parents_name', $student->parents_name) }}"
                ...
                <input id="location" name="location" value="{{ old('location', $student->location) }}" type="text"
                       class="form-control">
                @error('location')
                <div class="alert alert-danger" role="alert">{{ $message }}</div>
                @enderror
                ...
                <input id="description" name="description" value="{{ old('description', $student->description) }}"
                       type="number" class="form-control">


                <label for="should_pay" class="text-dark">Should Pay</label>
                <input id="should_pay" name="should_pay" type="text"
                       value="{{ number_format(old('should_pay', $student->should_pay ?? 0), 0, '', ' ') }}"
    ...
    <script>
        ...
        function formatBeforeSubmit() {
            let input = document.getElementById('should_pay');
