<section>
    ...
    <form method="post" action="{{ route('profile.save') }}"  enctype="multipart/form-data" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="name1" name="tel" type="text" class="mt-1 m-2 block w-full bg-white text-dark" :value="old('tel', $user->tel)" required autofocus autocomplete="tel" />
        </div>
        <div>
            <x-input-label for="name2" :value="__('Information not necessary')" />
            <x-text-input id="name2" name="desc" type="text" class="mt-1 m-2 block w-full bg-white text-dark" :value="old('desc', $user->desc)" />
        </div>
        <div>
